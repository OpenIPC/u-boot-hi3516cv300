...
int board_init(void)
{
	...
	gd->bd->bi_arch_number = MACH_TYPE_HI3519;
...
int save_bootdata_to_flash(void)
{
	...
	unsigned int  update_from_sdio0;
	void *buf;
	static struct spi_flash *flash;

	start_mode = readl(REG_BASE_SCTL + REG_SYSSTAT);
	update_from_sdio0 = (start_mode >> BIT_UPDATE_FROM_SDIO0)
				& UPDATE_FROM_SDIO0_MASK;
	if (update_from_sdio0) {
...
void set_pcie_para_hi3519()
{
	unsigned int val;

	val = readl(CRG_REG_BASE + PERI_CRG43);
	val = val & (~(0x1 << 0));
	writel(val, CRG_REG_BASE + PERI_CRG43);

	val = readl(CRG_REG_BASE + PERI_CRG43);
	val = val | (0x01 << 1);
	writel(val, CRG_REG_BASE + PERI_CRG43);

	writel(0x1a04, MISC_CTRL_REG_BASE + MISC_STRL33);
	writel(0x11a04, MISC_CTRL_REG_BASE + MISC_STRL33);
	writel(0x1a04, MISC_CTRL_REG_BASE + MISC_STRL33);
	writel(0x0, MISC_CTRL_REG_BASE + MISC_STRL33);
	
	writel(0xc09, MISC_CTRL_REG_BASE + MISC_STRL33);
	writel(0x10c09, MISC_CTRL_REG_BASE + MISC_STRL33);
	writel(0xc09, MISC_CTRL_REG_BASE + MISC_STRL33);
	writel(0x0, MISC_CTRL_REG_BASE + MISC_STRL33);
	
	writel(0x303, MISC_CTRL_REG_BASE + MISC_STRL33);
	writel(0x10303, MISC_CTRL_REG_BASE + MISC_STRL33);
	writel(0x303, MISC_CTRL_REG_BASE + MISC_STRL33);
	writel(0x0, MISC_CTRL_REG_BASE + MISC_STRL33);
}
